...
setwindowtitle (' График функции: 2 * x*x*x + 5 * x + 19');
SetFontColor(clgreen);
SetFontSize(10);
textout (30,30,'2 * x*x*x + 1 * x*x + 5 * x + 19');
...
begin
x1:=i/n;
y1:= 2 * power(x1, 3) + 1 * power(x1, 2) + 5 * x1 + 19;
line(trunc(x0), trunc(y0),trunc(400+x1*n),  trunc(400-y1*(n/10))
, clBlue);
...
x1h:=(i+h)/n;
y1h:= 2 * power(x1h, 3) + 1 * power(x1h, 2) + 5 * x1h + 19;
