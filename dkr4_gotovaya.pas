...
begin
  ...
  repeat
    a := readlnreal('Введите левую границу интервала от -10 до 0 a = ');
  until (a >= -10) and (a <= 0);
  repeat
...
begin
  ...
  textout(10, 70, '3 - Вывод площади ');
  textout(10, 90, '4 - Вывод площади по формуле Ньютона-Лейбница ');
  textout(10, 110, '5 - Вывод абсолютной погрешности ');
  textout(10, 130, '6 - Относительная погрешность ');
  textout(10, 150, '7 - График');
  textout(10, 170, '8 - Конец');
  read(a);
  clearwindow;
end;
...
begin
  ...
  begin
    case z of
      ...
      3: begin writeln('S=', s:0:5); if s = 0.00 then writeln('Параметры для высчитывания площади не заданы'); menu(z); readln; end;
      4: begin writeln('Аналитическое значение=', y:0:5);  menu(z); readln; end;
      5: begin writeln('Абсолютная погрешность=', abs(y - s):0:5);  menu(z); readln; end;
      6: begin writeln('Относительная погрешность=', abs(y - s) / y);  menu(z); readln; end;
      7: begin grafik(gx, gy, x0, y0, k, x1, y1, i, e, r, x2, x, y, h); readln; menu(z);  end;
      8: begin writeln('Спасибо за внимание');  writeln('Нажмите Enter чтобы выйти из приложения...'); exit; end;
    end;
  end;
end.